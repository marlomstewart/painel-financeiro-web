...
function App() {
  ...
  useEffect(() => {
    ...
    const carregarDadosDoBanco = async () => {
      try {
        const [resT, resC, resCat, resR, resF] = await Promise.all([
  ...
  // A LÓGICA ANTIGA DO REACT DE GERAR CONTAS FIXAS FOI REMOVIDA DAQUI! 
  // AGORA O ROBÔ NO NODE.JS CUIDA DISSO.
    ...
    try {
      ...
      if (data.auth) {
        ...
        if (data.precisaTrocar) {
          setTokenTemp(data.token); setPrecisaTrocarSenha(true);
        } else {
          localStorage.setItem('tokenPainel', data.token); setToken(data.token);
        }
  ...
  const enviarNovaSenha = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        localStorage.setItem('tokenPainel', tokenTemp); setToken(tokenTemp); setTokenTemp(null); setPrecisaTrocarSenha(false);
  ...
  const removerSetup = async (banco, id) => { let rota = banco === 'metasRenda' ? 'metas-renda' : banco === 'contasFixas' ? 'contas-fixas' : banco; await fetch(`https://painel-gestao-financeira-api.onrender.com/api/${rota}/${id}`, { method: 'DELETE', headers: headersAuth }); if (banco === 'cartoes') setCartoes(cartoes.filter(c => c.id !== id)); if (banco === 'categorias') setCategorias(categorias.filter(c => c.id !== id)); if (banco === 'metasRenda') setMetasRenda(metasRenda.filter(r => r.id !== id)); if (banco === 'contasFixas') setContasFixas(contasFixas.filter(f => f.id !== id)); };
  ...
  const addTransacao = async (e) => {
    ...
    if(c === 'Renda') t = 'renda'; if(c === 'Contas Fixas') t = 'despesa';
  ...
  // =========================================================================
  // CÁLCULOS E FILTROS DO DASHBOARD
  // =========================================================================
  ...
  const saldoAtual = totRendaTotal - totGastoPago;
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 text-slate-800">
      <div className="mx-auto max-w-6xl space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          <div className="bg-white p-4 rounded-xl shadow-sm border-l-4 border-emerald-500"><h3 className="text-xs font-semibold text-slate-500 uppercase">Rendas</h3><p className="text-lg font-bold mt-1">{formatarMoeda(totRendaTotal)}</p></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border lg:col-span-1 h-fit">
            ...
            <form onSubmit={addTransacao} className="space-y-4">
              ...
              <select name="categoria" required className="w-full border p-2.5 rounded-lg text-sm bg-white outline-none focus:border-blue-500"><option value="Sem Categoria">Sem Categoria</option><option value="Contas Fixas">Contas Fixas</option><option value="Renda">Renda</option>{categorias.map(c => <option key={c.id} value={c.nome}>{c.nome}</option>)}</select>
          ...
          <div className="bg-white rounded-xl shadow-sm border lg:col-span-2 flex flex-col">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left text-sm text-slate-600">
                <thead className="bg-white text-xs uppercase font-semibold border-b"><tr><th className="p-4">Descrição</th><th className="p-4">Categoria / Data</th><th className="p-4">Status / Pagto</th><th className="p-4 text-right">Valor</th><th className="p-4 text-center">Ações</th></tr></thead>
                <tbody className="divide-y divide-slate-100">
                  {transacoesFiltradas.length === 0 && (<tr><td colSpan="5" className="p-8 text-center text-slate-400">Nenhum lançamento encontrado.</td></tr>)}
                  {transacoesFiltradas.map(t => (
                    <tr key={t.id} className="hover:bg-slate-50 transition-colors">
                      <td className="p-4 font-bold text-slate-800 truncate max-w-[150px]">{t.descricao}</td>
                      ...
                      <td className="p-4 cursor-pointer" onClick={() => alternarStatusTransacao(t.id, t.status, t.valorParcela)}>
                        <div className="flex flex-col gap-1 items-start"><span className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${t.status === 'pago' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{t.status}</span><span className="text-[10px] uppercase text-slate-400 font-medium bg-slate-100 px-1.5 rounded">{t.formaPagamento ? t.formaPagamento.split('_')[0] : 'PIX'}</span></div>
                      </td>
                      <td className="p-4 font-bold text-slate-800 text-right">{formatarMoeda(t.valorParcela)}</td>
                      <td className="p-4">
                        <div className="flex justify-center gap-1"><button onClick={() => editarValor(t)} className="bg-white border text-slate-500 hover:bg-slate-100 px-2 py-1 rounded text-xs transition-colors">✏️</button><button onClick={() => deletarTransacao(t.id)} className="bg-red-50 text-red-500 hover:bg-red-100 px-2 py-1 rounded text-xs transition-colors">🗑️</button></div>
                      </td>
                    </tr>
                  ))}
                </tbody>
