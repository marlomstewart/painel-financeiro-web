...
export function Cartoes({ cartoes, addCartao, editarSetup, removerSetup, modal }) {
    ...
    const handleSubmit = async (e) => {
        await addCartao(e);
        setNomeCartao(''); setLimite(''); setDiaFechamento(''); setDiaVencimento('');
    };

    const handleEditar = async (c) => {
        const nNome = await modal.prompt(`1️⃣ Novo NOME do Cartão?`, c.nome, '✏️ Editar Cartão', { confirmLabel: 'Próximo' }); if (nNome === null) return;
        const nLim = await modal.prompt(`2️⃣ Novo LIMITE (R$)?`, String(c.limite), '✏️ Editar Cartão', { inputType: 'number', confirmLabel: 'Próximo' }); if (nLim === null) return;
        const nF = await modal.prompt(`3️⃣ Novo Dia de FECHAMENTO?`, String(c.diaFechamento), '✏️ Editar Cartão', { inputType: 'number', confirmLabel: 'Próximo' }); if (nF === null) return;
        const nV = await modal.prompt(`4️⃣ Novo Dia de VENCIMENTO?`, String(c.diaVencimento), '✏️ Editar Cartão', { inputType: 'number', confirmLabel: 'Salvar' }); if (nV === null) return;
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div className="lg:col-span-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 rounded-xl shadow-sm h-fit">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Limite Total (R$)</label><input name="limite" type="number" step="0.01" min="0.01" value={limite} onChange={(e) => setLimite(e.target.value)} required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500" placeholder="0.00" /></div>
                        <div className="grid grid-cols-2 gap-3">
                            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Dia Fechamento</label><input name="melhorDia" type="number" min="1" max="31" value={diaFechamento} onChange={(e) => setDiaFechamento(e.target.value)} required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500" /></div>
                            <div><label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1">Dia Vencimento</label><input name="vencimento" type="number" min="1" max="31" value={diaVencimento} onChange={(e) => setDiaVencimento(e.target.value)} required className="w-full bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg p-2.5 text-sm text-slate-800 dark:text-slate-200 outline-none focus:border-blue-500" /></div>
                        </div>
                        ...
                        cartoes.map(c => (
                            <div key={c.id} className="bg-gradient-to-tr from-slate-800 to-slate-900 text-white p-5 rounded-2xl shadow-lg border border-slate-700 relative overflow-hidden group">
                                ...
                                <div className="flex justify-between items-start mb-6 relative z-10">
                                    ...
                                    {/* ATUALIZAÇÃO TOUCH: Sempre visível no mobile (opacity-100), se esconde apenas em Desktop (md:opacity-0) aguardando o Hover */}
