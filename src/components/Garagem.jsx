...
export function Garagem({ getHeaders, setTelaAtiva, transacoes, setTransacoes, cartoes = [], ModalComponent, modalConfig, modalClose, garagem, dataVis }) {
    ...
    const [periodoAbastecimentos, setPeriodoAbastecimentos] = useState('todo');
    ...
    const precoLitroCalculado = valorLancamentoVinculado > 0 && Number(rascunhoAbastecimento.litros) > 0
        ...
        : '';
    const litrosRascunho = rascunhoAbastecimento.modo === 'vincular' && rascunhoAbastecimento.campoBase === 'precoLitro'
        ? litrosCalculados
    ...
    const abastecimentosVisiveis = (() => {
        const ordenados = [...abastecimentos].sort((a, b) => new Date(b.data_abastecimento) - new Date(a.data_abastecimento));
        if (periodoAbastecimentos === 'todo') return ordenados;
        const limite = new Date();
        limite.setUTCHours(0, 0, 0, 0);
        if (periodoAbastecimentos === '30') limite.setUTCDate(limite.getUTCDate() - 29);
        if (periodoAbastecimentos === '90') limite.setUTCMonth(limite.getUTCMonth() - 3);
        return ordenados.filter(abastecimento => new Date(`${abastecimento.data_abastecimento}T00:00:00Z`) >= limite);
    })();
            ...
            <div className={`grid grid-cols-1 ${veiculoSelecionado.tipo !== 'convidado' ? 'lg:grid-cols-2' : ''} gap-5 md:gap-6`}>
                    ...
                    <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
                        ...
                        {!consumoCombustivel ? <p className="text-sm text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg p-5 text-center">Carregando dados técnicos de consumo...</p> : <>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                ...
                                <div className="rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700 p-3"><span className="block text-[10px] font-bold uppercase tracking-wider text-slate-500">Custo médio por dia útil</span><strong className="block text-sm text-slate-800 dark:text-slate-100 mt-1">{consumoCombustivel.custo_medio_diario === null ? 'Dados insuficientes' : formatarMoeda(consumoCombustivel.custo_medio_diario)}</strong><span className="block text-[10px] text-slate-500 mt-1">{consumoCombustivel.dias_uteis_observados ? `${consumoCombustivel.dias_uteis_observados} dias úteis em Aracaju/SE` : 'Sem período útil observado'}</span></div>
                ...
                {veiculoSelecionado.tipo !== 'convidado' && (
                    <div className="bg-white dark:bg-slate-800 p-5 md:p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
                        ...
                        {abastecimentos.length > 0 && <div className="flex gap-2 mb-3" role="group" aria-label="Período do histórico de abastecimentos">{[{ id: '30', nome: '30 dias' }, { id: '90', nome: '3 meses' }, { id: 'todo', nome: 'Todo histórico' }].map(periodo => <button key={periodo.id} type="button" onClick={() => setPeriodoAbastecimentos(periodo.id)} className={`rounded-md px-2.5 py-1.5 text-[10px] font-bold transition-colors ${periodoAbastecimentos === periodo.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600'}`}>{periodo.nome}</button>)}</div>}
                        {abastecimentos.length === 0 ? <p className="text-sm text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg p-6 text-center">Nenhum abastecimento técnico registrado.</p> : abastecimentosVisiveis.length === 0 ? <p className="text-sm text-slate-500 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-lg p-6 text-center">Nenhum abastecimento neste período.</p> : <div className="space-y-2 max-h-[330px] overflow-y-auto custom-scrollbar">{abastecimentosVisiveis.map(a => <div key={a.id} className="p-3 rounded-lg bg-slate-50 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-700"><div className="flex justify-between gap-3"><strong className="text-sm text-slate-800 dark:text-slate-100">{formatarData(a.data_abastecimento)} · {Number(a.odometro).toLocaleString('pt-BR')} km</strong><div className="flex items-center gap-2"><span className="font-black text-rose-600">{formatarMoeda(a.valor_total)}</span><button type="button" onClick={() => solicitarExclusaoAbastecimento(a)} className="p-1.5 text-slate-400 hover:text-rose-600 dark:hover:text-rose-400 rounded hover:bg-slate-100 dark:hover:bg-slate-700 transition-colors cursor-pointer" title="Excluir abastecimento"><Trash2 className="w-3.5 h-3.5" strokeWidth={2} /></button></div></div><p className="text-[11px] text-slate-500 mt-1">{Number(a.litros).toLocaleString('pt-BR')} L · {formatarMoeda(a.preco_litro)}/L · {a.tanque_cheio ? (a.km_por_litro ? `${a.km_por_litro} km/L` : 'tanque cheio') : 'abastecimento parcial'}</p>{a.observacao && <p className="text-[11px] text-slate-500 italic mt-1">{a.observacao}</p>}</div>)}</div>}
                    </div>
                )}
