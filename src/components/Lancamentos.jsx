                ...
                {transacoesSelecionadas.length > 0 && (
                    <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800/50 rounded-lg p-3 md:p-4 mb-4 flex flex-col sm:flex-row items-center justify-between gap-4 animate-fade-in">
                        ...
                        <div className="flex flex-wrap gap-2 justify-center w-full sm:w-auto">
                            <button type="button" onClick={async () => { if (await executarAcaoEmMassa(transacoesSelecionadas, 'pago')) setTransacoesSelecionadas([]); }} className="flex-1 sm:flex-none px-4 py-2.5 md:py-1.5 bg-emerald-600 text-white text-xs font-bold rounded-lg hover:bg-emerald-700 cursor-pointer transition shadow-sm">Marcar Pago</button>
                            <button type="button" onClick={async () => { if (await executarAcaoEmMassa(transacoesSelecionadas, 'pendente')) setTransacoesSelecionadas([]); }} className="flex-1 sm:flex-none px-4 py-2.5 md:py-1.5 bg-amber-500 text-white text-xs font-bold rounded-lg hover:bg-amber-600 cursor-pointer transition shadow-sm">Marcar Pendente</button>
                            <button type="button" onClick={async () => { if (await executarAcaoEmMassa(transacoesSelecionadas, 'excluir')) setTransacoesSelecionadas([]); }} className="w-full sm:w-auto px-4 py-2.5 md:py-1.5 bg-rose-600 text-white text-xs font-bold rounded-lg hover:bg-rose-700 cursor-pointer transition shadow-sm border border-rose-700">Excluir</button>
                        </div>
                    </div>
                )}
