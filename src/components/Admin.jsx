...
export function Admin({ ModalComponent, modalConfig, modalClose, setTelaAtiva, criarUsuario, carregarUsuarios, usuarios, toggleAdmin, resetarSenha, deletarUsuario }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 p-4 md:p-6 text-slate-800 dark:text-slate-200 transition-colors duration-300 relative">
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <div className="sticky top-0 z-40 pt-4 md:pt-6 pb-2 -mt-4 md:-mt-6 bg-slate-50/90 dark:bg-slate-900/90 backdrop-blur-md mb-6">
          <header className="border-b border-slate-200 dark:border-slate-700 pb-4 flex justify-between items-center transition-colors">
            ...
            <button type="button" onClick={() => setTelaAtiva('dashboard')} className="bg-slate-900 dark:bg-slate-700 text-white font-medium py-2 px-6 rounded-lg hover:bg-slate-800 dark:hover:bg-slate-600 transition-colors cursor-pointer shadow-sm">Voltar</button>
          </header>
        </div>
        ...
        <section className="bg-white dark:bg-slate-800 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 transition-colors">
          ...
          <form onSubmit={criarUsuario} className="flex flex-col md:flex-row gap-4 items-end">
            ...
            <div className="flex items-center gap-2 pb-3 w-full md:w-auto justify-start md:justify-center">
            ...
            <button type="submit" className="w-full md:w-auto bg-blue-600 dark:bg-indigo-600 hover:bg-blue-700 dark:hover:bg-indigo-700 text-white font-bold py-3 px-6 rounded-lg text-sm transition-colors cursor-pointer shadow-md">Criar Usuário</button>
          </form>
        ...
        <section className="bg-white dark:bg-slate-800 rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 overflow-hidden transition-colors flex flex-col">
          <div className="p-5 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-900/50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 transition-colors">
            ...
            <button
              ...
              className={`text-xs font-bold px-3 py-1.5 rounded-lg border transition-colors flex items-center justify-center gap-2 ${atualizando ? 'bg-slate-200 dark:bg-slate-700 text-slate-500 border-slate-300 dark:border-slate-600 cursor-wait' : 'bg-white dark:bg-slate-800 text-blue-600 dark:text-blue-400 border-slate-200 dark:border-slate-700 hover:bg-blue-50 dark:hover:bg-slate-700 cursor-pointer shadow-sm'}`}
            >
              {atualizando ? (
          ...
          {usuarios.length === 0 ? (
            <div className="p-12 text-center text-slate-400 dark:text-slate-500 text-sm flex flex-col items-center gap-3">
              ...
              <button type="button" onClick={handleAtualizarLista} className="text-blue-500 hover:underline">Clique aqui para sincronizar agora.</button>
            </div>
          ) : (
            <div className="overflow-x-auto w-full custom-scrollbar">
              <table className="w-full text-sm text-left">
                <thead className="bg-slate-100/50 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700 text-[10px] uppercase tracking-wider text-slate-500 dark:text-slate-400 font-black transition-colors whitespace-nowrap">
                  <tr>
                    <th className="px-5 py-4">Utilizador Registado</th>
                    <th className="px-5 py-4 text-center">Nível de Acesso</th>
                    <th className="px-5 py-4 text-center">Última Sessão (Login)</th>
                    <th className="px-5 py-4 text-center">Status da Credencial</th>
                    <th className="px-5 py-4 text-center">Ações Destrutivas</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-slate-100 dark:divide-slate-700">
                  {usuarios.map(u => (
                    <tr key={u.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                      ...
                      <td className="px-5 py-4 font-black text-slate-800 dark:text-slate-200 whitespace-nowrap">
                        <div className="flex items-center gap-2">
                          <div className="w-8 h-8 rounded-full bg-slate-200 dark:bg-slate-700 flex items-center justify-center text-slate-500 font-bold uppercase">
                            {u.usuario.charAt(0)}
                          </div>
                          {u.usuario}
                        </div>
                      ...
                      <td className="px-5 py-4 text-center whitespace-nowrap">
                        <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md shadow-sm ${u.is_admin === 1 ? 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800/50' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-700'}`}>
                          {u.is_admin === 1 ? '⭐ Administrador' : 'Visualizador'}
                        </span>
                      </td>

                      <td className="px-5 py-4 text-center whitespace-nowrap">
                        {/* Busca por múltiplos padrões de resposta da API */}
                        {formatarUltimoAcesso(u.ultimo_login || u.ultimoLogin || u.created_at || u.createdAt || null)}
                      </td>

                      <td className="px-5 py-4 text-center whitespace-nowrap">
                        <span className={`text-[10px] font-black uppercase tracking-wider px-2.5 py-1 rounded-md shadow-sm ${u.precisa_trocar === 1 ? 'bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400 border border-amber-200 dark:border-amber-800/50' : 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/50'}`}>
                          {u.precisa_trocar === 1 ? '⚠ VAZADA (Padrão)' : '✔ FORTE (Própria)'}
                        </span>
                      </td>

                      <td className="px-5 py-4">
                        <div className="flex gap-2 justify-center">
                          <button type="button" onClick={() => toggleAdmin(u.id, u.usuario, u.is_admin === 1)} title={u.is_admin === 1 ? 'Rebaixar para Visualizador' : 'Promover a Administrador'} className="text-sm bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400 hover:bg-purple-100 dark:hover:bg-purple-900/50 border border-purple-200 dark:border-purple-800 px-3 py-1.5 rounded-lg font-bold cursor-pointer transition-colors">
                            {u.is_admin === 1 ? '↓ Rebaixar' : '↑ Promover'}
                          </button>
                          <button type="button" onClick={() => resetarSenha(u.id, u.usuario)} title="Forçar a senha do utilizador a voltar para 'admin123'" className="text-sm bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 hover:bg-amber-100 dark:hover:bg-amber-900/50 border border-amber-200 dark:border-amber-800 px-3 py-1.5 rounded-lg font-bold cursor-pointer transition-colors">
                            🔑 Reset
                          </button>
                          <button type="button" onClick={() => deletarUsuario(u.id, u.usuario)} title="Apagar a conta do utilizador permanentemente" className="text-sm bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400 hover:bg-rose-100 dark:hover:bg-rose-900/50 border border-rose-200 dark:border-rose-800 px-3 py-1.5 rounded-lg font-bold cursor-pointer transition-colors">
                            🗑️ Excluir
                          </button>
                        </div>
                      </td>
