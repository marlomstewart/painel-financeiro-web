 ...
 * Empilha múltiplos toasts simultâneos, o mais recente embaixo.
 */
export function Toast({ toasts = [] }) {
    if (!toasts.length) return null;

    return (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:bottom-6 md:right-6 md:max-w-md flex flex-col gap-2.5 z-[9999] pointer-events-none">
            {toasts.map(toast => {
                ...
                return (
                    <div key={toast.id} className={`flex items-center gap-3 md:gap-4 ${bgClass} text-white px-5 md:px-6 py-4 md:py-3.5 rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.3)] border transition-all animate-fade-in-up backdrop-blur-md pointer-events-auto`}>
